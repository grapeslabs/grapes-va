...
migrations: List[Tuple[str, str, str]] = [
    ...
    (
        "ALTER TABLE analytics_events ALTER COLUMN person_photobank_id DROP NOT NULL;",
        "pacs",
        "Убрать NOT NULL ограничение с колонки person_photobank_id в analytics_events. "
        "Колонка допускает NULL для лиц, не распознанных системой.",
    ),
]
...
def apply_migrations(
    ...
    if not current_file.endswith(".py") or not os.path.isfile(current_file):
        log_func(
            "debug", f"Файл миграций уже обработан или отсутствует: {current_file}"
        )
        return False
    ...
    for sql, mode_required, desc in migrations:
        if mode_required == "" or mode_required == mode:
            ...
            log_func(
                "info",
                f"Миграция пропущена (режим '{mode_required}' не соответствует '{mode}'): {desc}",
            )
    ...
    log_func(
        "info", f"Найдено {len(filtered)} миграций для режима '{mode}', применяем..."
    )
    ...
    if error_count:
        log_func(
            "warning",
            f"Выполнено {success_count} миграций, ошибок: {error_count}. Файл всё равно будет переименован.",
        )
    else:
...
def _rename_self(file_path: str, log_func: Callable) -> None:
    ...
    try:
        ...
        log_func(
            "info", f"Файл миграций переименован в {os.path.basename(backup_path)}"
        )
    except Exception as e:
